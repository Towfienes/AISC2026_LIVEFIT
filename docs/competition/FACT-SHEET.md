...
| Intent classifier | macro-F1 0,870 — **trên 320 câu mẫu do AI (Claude) soạn ngày 01/09, 5-fold CV** | `python -m livelift.nlp.train_intent` |
...
| Bộ kiểm thử | **1.555 test nhanh xanh + 17 cổng Monte-Carlo + 10 test trình duyệt thật = 1.582** (đếm 17/09/2026 bằng `scripts/dong_bo_so_test.py --xem-truoc`, sau đợt kiểm toán toàn diện 17/09 — trước đó 1.157 + 17 = 1.174 ngày 15/09) | `pytest -m "not slow"` và `pytest -m slow`; **chạy `scripts/dong_bo_so_test.py --xem-truoc` trước mỗi lần nộp** — nó lấy pytest làm nguồn duy nhất và tự báo chỗ lệch |
| Sổ sự cố | **58 sự cố có nguyên nhân gốc** (đếm 17/09/2026 — thêm: tick luôn 0 lượt bấm và bình luận, nhấp đúp vượt xác nhận kết thúc phiên, Facebook dò buổi live một lần, bình luận mô phỏng lọt lô gán nhãn, khoá YouTube trong log, README còn số A/A cũ, kho bộ nhớ lưu trùng dưới tải, WebSocket nhận trễ, link đo đếm thiếu sau proxy, compose không truyền INGEST_TOKEN, đường Shopee trả 422) | đếm số hàng bảng trong `docs/incident-log.md` |
